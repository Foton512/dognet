  ...
  "$compile", function($compile) {
    ...
    return {
      ...
      resetYielder: function(name) {
        var b = yielders[name];
		  if (b.defaultReset || b.needReset) {
			  this.setContentFor(name, b.defaultContent, b.defaultScope);
		  }
      },

      putYielder: function(name, element, defaultScope, defaultContent, defaultReset) {
        var yielder = {};
        yielder.name = name;
        yielder.element = element;
        yielder.defaultContent = defaultContent || "";
        yielder.defaultScope = defaultScope;
		yielder.defaultReset = defaultReset !== "false";
		yielder.needReset = false;
      ...
      setContentFor: function(name, content, scope) {
		...
		if (!b.defaultReset) { b.needReset = true; }
  ...
  "$compile", "CaptureService", function($compile, CaptureService) {
    return {
      link: function(scope, element, attr) {
        CaptureService.putYielder(attr.yieldTo, element, scope, element.html(), attr.defaultReset);
