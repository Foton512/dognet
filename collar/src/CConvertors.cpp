...
//=============================================================================
//! Convert cp866 string to koi8-r string
//!
//! \param cp866_str String to convert
//=============================================================================
std::string CConvertors::convertCp866ToKoi8R(const std::string &cp866_str)
{
	// cp866 to koi8-r codes table
	static const int8_t cp866_to_koi8r[] =
	{
			...
			0xE0, 0xF1, 0xC1, 0xC2, 0xD7, 0xC7, 0xC4, 0xC5, 0xD6, 0xDA, 0xC9, 0xCA, 0xCB, 0xCC, 0xCD,
			0xCE, 0xCF, 0xD0, 0x90, 0x91, 0x92, 0x81, 0x87, 0xB2, 0xB4, 0xA7, 0xA6, 0xB5, 0xA1, 0xA8,
			0xAE, 0xAD, 0xAC, 0x83, 0x84, 0x89, 0x88, 0x86, 0x80, 0x8A, 0xAF, 0xB0, 0xAB, 0xA5, 0xBB,
			0xB8, 0xB1, 0xA0, 0xBE, 0xB9, 0xBA, 0xB6, 0xB7, 0xAA, 0xA9, 0xA2, 0xA4, 0xBD, 0xBC, 0x85,
			0x82, 0x8D, 0x8C, 0x8E, 0x8F, 0x8B, 0xD2, 0xD3, 0xD4, 0xD5, 0xC6, 0xC8, 0xC3, 0xDE,	0xDB,
			0xDD, 0xDF, 0xD9, 0xD8, 0xDC, 0xC0, 0xD1, 0xB3, 0xA3, 0x98, 0x99, 0x97, 0x93, 0x9B, 0x9D,
			0x9C, 0x95,	0x9E, 0x96, 0x9F, 0xBF, 0x94, 0x9A
	};

	std::string koi8r_str = cp866_str;

	for (size_t i = 0; i < koi8r_str.size(); ++i)
	{
		if (static_cast<unsigned char> (koi8r_str[i]) >= 0x80)
		{
			koi8r_str[i] = cp866_to_koi8r[static_cast<unsigned char> (koi8r_str[i]) - 0x80];
		}
	}
	return koi8r_str;
}
...
//=============================================================================
//! Convert cp866 string to cp1251 string
//!
//! \param cp866_str string to convert
//=============================================================================
std::string CConvertors::convertCp866ToCp1251(const std::string &cp866_str)
{
	// cp866 to cp1251 codes table (0x01 used for missed symbols, except first symbol
	static const int8_t cp866_to_cp1251[] =
	{
		...
		0xA8, 0xB8, 0xAA, 0xBA, 0xAF, 0xBF, 0xA1, 0xA2, 0xB0, 0x01, 0xB7, 0x01, 0x01, 0xA4, 0x01, 0xA0,
	};

	std::string cp1251_str = cp866_str;

	for (size_t i = 0; i < cp1251_str.size(); ++i)
	{
		if (static_cast<unsigned char>(cp1251_str[i]) >= 0x80)
		{
			cp1251_str[i] = cp866_to_cp1251[static_cast<unsigned char>(cp1251_str[i]) - 0x80];
		}
	}
